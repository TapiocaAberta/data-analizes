...
public class BaixaDocumentoDeEmenda {
        ...
        private static final int QUANTIDADE_POR_PAGINA = 1000;
        ...
        private static List<Documento> buscaDocumentos(String codigoEmenda)
        ...
        private static List<Documento> buscaDocumentos(final int offset, final String codigoEmenda)
                        throws URISyntaxException, IOException, InterruptedException {
                // busca documentos
                ...
                var response = HttpClient.newHttpClient().send(request, BodyHandlers.ofString()).body();

                var data = MAPPER.readTree(response).get("data");
                var listaDocumentos = MAPPER.readValue(data.toString(), DocumentoBusca[].class);
        ...
        private static Documento scrapDetalhesDoDocumento(DocumentoBusca documentoBusca) {
                try {
                        ...
                        var orgapSuperiorDiv = pagadorDiv.select("strong:contains(Órgão Superior)");
                        ...
                        var orgaoSuperiorCod = orgapSuperiorDiv.next("span").text();
                        var orgaoSuperiorNome = orgapSuperiorDiv.next("span").next("span").text();
                        var url = orgapSuperiorDiv.next("span").next("span").select("a").attr("abs:href");
